...
use std::fs;
use crate::core::{models::Group, error::AppError};
use crate::core::db_state::DbState;
use crate::commands::keyword::{convert_to_pinyin, convert_to_acronym};
use crate::core::meme_fs::{validate_name, sanitize_folder_name};

/// 获取模式对应的文件夹路径
fn get_mode_folder_path(conn: &rusqlite::Connection, mode_id: i32) -> Result<String, AppError> {
...
/// 构建分组的完整文件夹路径
/// 分组文件夹放在对应模式的文件夹下
fn build_group_folder_path(conn: &rusqlite::Connection, mode_id: i32, name: &str) -> Result<String, AppError> {
    let mode_path = get_mode_folder_path(conn, mode_id)?;
    let safe_name = sanitize_folder_name(name);
    Ok(PathBuf::from(mode_path).join(safe_name).to_string_lossy().to_string())
}

#[tauri::command]
pub fn get_groups_by_mode(state: tauri::State<'_, DbState>, mode_id: i32) -> Result<Vec<Group>, AppError> {
...
#[tauri::command]
pub fn get_all_groups(state: tauri::State<'_, DbState>) -> Result<Vec<Group>, AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    let mut stmt = conn.prepare(
        "SELECT g.id, g.name, g.folder_path, g.share_count, g.mode_id, m.name 
         FROM groups g 
...
#[tauri::command]
pub fn get_all_groups_simple(state: tauri::State<'_, DbState>) -> Result<Vec<Group>, AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    let mut stmt = conn.prepare(
        "SELECT g.id, g.name, g.folder_path, g.share_count, g.mode_id, m.name 
         FROM groups g 
...
#[tauri::command]
pub fn add_group(state: tauri::State<'_, DbState>, name: String, mode_id: i32) -> Result<i32, AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    // 验证名称
    validate_name(&name)?;
    
    // 构建文件夹路径（名称即文件夹名）
    let folder_path = build_group_folder_path(&conn, mode_id, &name)?;
...
#[tauri::command]
pub fn update_group(state: tauri::State<'_, DbState>, id: i32, name: String, mode_id: i32) -> Result<(), AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    // 验证名称
    validate_name(&name)?;
    ...
    // 构建新的文件夹路径
    let new_folder_path = build_group_folder_path(&conn, mode_id, &name)?;
...
#[tauri::command]
pub fn delete_group(state: tauri::State<'_, DbState>, group_id: i32) -> Result<(), AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    delete_group_internal(&conn, group_id)
}

#[tauri::command]
pub fn delete_groups(state: tauri::State<'_, DbState>, group_ids: Vec<i32>) -> Result<(), AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    for id in group_ids {
...
pub fn search_groups(
    ...
    if keyword.trim().is_empty() {
        if let Some(mid) = mode_id {
            return get_groups_by_mode(state, mid);
        } else {
...
/// 重建所有分组的关键词关联
#[tauri::command]
pub fn rebuild_all_group_keywords(state: tauri::State<'_, DbState>) -> Result<i32, AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
