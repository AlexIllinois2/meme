...
/// 从文件加载关键词配置
fn load_keywords_from_file(meme_dir: String) -> Result<KeywordsToml, AppError> {
    ...
    let content = std::fs::read_to_string(&keywords_file)
        .map_err(|e| AppError(format!("Failed to read keywords.toml: {}", e)))?;

    let keywords_toml: KeywordsToml = toml::from_str(&content)
        .map_err(|e| AppError(format!("Failed to parse keywords.toml: {}", e)))?;
...
/// 根据模式/分组文件夹名自动生成关键词
#[tauri::command]
pub fn generate_keywords_file(meme_dir: String, generate_pinyin: bool, generate_acronym: bool) -> Result<(), AppError> {
    use std::fs;

    let meme_path = PathBuf::from(&meme_dir);

    if !meme_path.exists() {
        return Err(format!("表情包目录不存在: {}", meme_dir).into());
    }
    ...
    let entries = fs::read_dir(&meme_path)
        .map_err(|e| AppError(format!("无法读取目录: {}", e)))?;

    for entry in entries {
        let entry = entry.map_err(|e| AppError(format!("读取目录项失败: {}", e)))?;
        let path = entry.path();
        ...
        let group_entries = fs::read_dir(&path)
            .map_err(|e| AppError(format!("无法读取模式目录 {}: {}", mode_name, e)))?;

        for group_entry in group_entries {
            let group_entry = group_entry.map_err(|e| AppError(format!("读取分组目录失败: {}", e)))?;
    ...
    let mut doc = if keywords_file.exists() {
        ...
        content.parse::<toml_edit::DocumentMut>()
            .map_err(|e| AppError(format!("Failed to parse TOML: {}", e)))?
    } else {
        // 创建新文档
    ...
    std::fs::write(&keywords_file, content)
        .map_err(|e| AppError(format!("Failed to write keywords file: {}", e)))?;
...
#[tauri::command]
pub fn import_keywords_from_file(state: tauri::State<'_, DbState>, meme_dir: String) -> Result<(), AppError> {
...
#[tauri::command]
pub fn sync_keywords_to_file(state: tauri::State<'_, DbState>) -> Result<(), AppError> {
    // 从配置中获取 meme_dir
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
...
#[tauri::command]
pub fn get_keywords_by_group_name(state: tauri::State<'_, DbState>, group_name: String) -> Result<Vec<String>, AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
...
/// 从所有同名分组中移除与指定关键词的关联
#[tauri::command]
pub fn remove_keyword_from_group_name(state: tauri::State<'_, DbState>, keyword: String, group_name: String) -> Result<(), AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
    ...
    let keyword_id: i32 = conn.query_row(
        ...
        params![keyword],
        |row| row.get(0)
    ).map_err(|_| AppError(format!("关键词 '{}' 不存在", keyword)))?;
...
/// 如果关键词不存在则新建，存在则直接使用
#[tauri::command]
pub fn add_keyword_to_group_name(state: tauri::State<'_, DbState>, keyword: String, group_name: String) -> Result<(), AppError> {
    let conn = state.lock().map_err(|e| AppError(e.to_string()))?;
