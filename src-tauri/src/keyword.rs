...
#[tauri::command]
pub fn import_keywords_from_file(meme_dir: String) -> Result<(), AppError> {
    let conn = init_db()?;
...
#[tauri::command]
pub fn sync_keywords_to_file() -> Result<(), AppError> {
    // 从配置中获取 meme_dir
    let conn = init_db()?;
...
#[tauri::command]
pub fn get_keywords_by_group_name(group_name: String) -> Result<Vec<String>, AppError> {
    let conn = init_db()?;
...
/// 从所有同名分组中移除与指定关键词的关联
#[tauri::command]
pub fn remove_keyword_from_group_name(keyword: String, group_name: String) -> Result<(), AppError> {
    let conn = init_db()?;
...
/// 如果关键词不存在则新建，存在则直接使用
#[tauri::command]
pub fn add_keyword_to_group_name(keyword: String, group_name: String) -> Result<(), AppError> {
    let conn = init_db()?;
